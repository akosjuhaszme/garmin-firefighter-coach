...
WORKOUT_STRUCTURE_REFERENCE = {
    ...
    "strength_step_fields": {
        ...
        "known_category_exerciseName_pairs": {
            "_source": "Cross-confirmed by two independent sources: docs.tryterra.co/planned-workouts-api/garmin-exercise-reference, and github.com/mrnabilnoh/workout-plan-garmin-connect (which is built directly from a workouts.json pulled from Garmin Connect itself). Both agree on category=HIP_STABILITY / exerciseName=QUADRUPED_WITH_LEG_LIFT for bird dog, with no literal 'BIRD_DOG' entry in Garmin's catalog. Not yet live-verified via upload+app-screenshot the way category=SQUAT/weight were, but two independently-sourced agreeing references is stronger footing than the SQUAT-only case had before it was live-confirmed. If a common exercise name doesn't match Garmin's category directly, it's usually filed under a less obvious category - check here before assuming it doesn't exist.",
